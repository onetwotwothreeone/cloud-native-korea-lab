...
def test_portfolio_demo_readiness_doc_exists():
    doc = ROOT / "docs" / "69_GWAN_Kubernetes_StatefulSet_Portfolio_Demo_Readiness_Report.md"
    assert doc.exists()

    text = doc.read_text(encoding="utf-8")
    assert "Portfolio_Demo_Readiness_Report" in text
    ...
    assert "70_GWAN_Kubernetes_StatefulSet_Portfolio_Demo_Script" in text


def test_portfolio_demo_readiness_prompt_exists():
    prompt = ROOT / "code" / "69_gwan_kubernetes_statefulset_portfolio_demo_readiness_report_prompt.md"
    assert prompt.exists()

    text = prompt.read_text(encoding="utf-8")
    assert "portfolio demo readiness report" in text.lower()
    assert "DEMO_STATUS=READY_FOR_PORTFOLIO_REVIEW" in text
    assert "MIGRATION_EXECUTION_ALLOWED=false" in text
    assert "REAL_MIGRATION_EXECUTED=false" in text
    assert "SECRET_VALUES_EXPORTED=false" in text
...
def test_portfolio_demo_readiness_script_exists():
    script = ROOT / "scripts" / "k8s" / "statefulset_portfolio_demo_readiness_report.sh"
...
def test_portfolio_demo_readiness_script_does_not_execute_migration():
    script = ROOT / "scripts" / "k8s" / "statefulset_portfolio_demo_readiness_report.sh"
    text = script.read_text(encoding="utf-8")
