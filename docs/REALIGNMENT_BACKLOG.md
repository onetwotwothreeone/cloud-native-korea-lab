...
## 추후 운영 셋업 (Deferred Ops Setup)

> 코드/정체성 정렬이 아니라 **프로젝트 운영 자동화** 항목. 지금은 기록만 하고 구현하지 않는다(선택과 집중 — 예방 레이어 먼저).

- [ ] **OPS1. 닻 리마인더 자동화** *(심각도: Low)*
  - 목적: [MOTIVATION.md](MOTIVATION.md) 4절의 점검 질문 3개(① 수단이 목적을 밀어냈는가 ② `hypothesis`를 `confirmed`처럼 다루는가 ③ 억지로 미는가/따라 흐르는가)를 **정기적으로 자동 제시**해, 방향이 흔들릴 때 닻으로 되돌아오게 함.
  - 방식 후보: **Claude Cloud Routine**(기기가 꺼져 있어도 클라우드에서 실행) + 결과를 **이메일/캘린더**로 수신.
  - 착수 시점: **예방 레이어 작업 완료 후 PR 머지되는 정비 지점**.
  - 지금은 **구현하지 않고 기록만**.
